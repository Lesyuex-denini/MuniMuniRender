...
def recommendations_view(request):
    ...
    user = request.user
    all_recommendations = generate_personalized_recommendations(
        user, category=category, query=query
    )
    ...
    user_recommendations = UserRecommendation.objects.filter(
        user=user, recommendation__in=unique_recommendations
    ...
    for rec in unique_recommendations:
        status = interaction_status.get(
            rec.id, {"liked": False, "done": False, "saved": False}
        )
    ...
    if request.method == "POST" and "ai_input" in request.POST:
        ...
        if ai_recommendation_obj:
            existing_recommendation = Recommendation.objects.filter(
                title=ai_recommendation_obj.title,
                description=ai_recommendation_obj.description,
            ).first()
            ...
            user_recommendation, created = UserRecommendation.objects.get_or_create(
                user=request.user, recommendation=ai_recommendation_obj
            )
            ai_recommendation = {
...
def like_recommendation(request, recommendation_id):
    if request.method == "POST":
        ...
        user_recommendation, created = UserRecommendation.objects.get_or_create(
            user=request.user, recommendation=recommendation
        )
...
def saved_recommendations_view(request):
    saved_recommendations = (
        UserRecommendation.objects.filter(user=request.user, saved=True)
...
def add_to_planner_view(request, recommendation_id):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            messages.success(
                request, f"'{recommendation.title}' added to your planner."
            )
